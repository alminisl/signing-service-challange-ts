import { createSignatureDevice } from "../src/api/createSignatureDevice";
...
describe("signNewDevice", () => {
  ...
  it("should throw an error if device ID is not provided", async () => {
    await expect(
      createSignatureDevice("", AlgorithmEnum.EC, persistence)
  ...
  it("should throw an error if label is too long", async () => {
    await expect(
      createSignatureDevice(
        ...
        AlgorithmEnum.EC,
        persistence,
        "this is a very long label that exceeds the maximum length of 50 characters"
      )
    ).rejects.toThrow("Label exceeds maximum length of 50 characters");
  });
  ...
  it("should throw an error if device with same ID already exists", async () => {
    ...
    await expect(
      createSignatureDevice("device1", AlgorithmEnum.EC, persistence)
