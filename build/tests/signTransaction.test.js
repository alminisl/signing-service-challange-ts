...
describe("signTransaction", () => {
    ...
    it("should sign a transaction with a valid device", async () => {
        ...
        const expectedData = `0_${data}_${Buffer.from(deviceId).toString("base64")}`;
    ...
    it("should throw an error if the device ID is invalid", async () => {
        ...
        await expect(signTransaction(invalidDeviceId, data, persistence)).rejects.toThrowError("Invalid deviceId");
    });
    it("should throw an error if the data is invalid", async () => {
        const invalidData = "";
        await expect(signTransaction(deviceId, invalidData, persistence)).rejects.toThrowError("Invalid data");
    });
    it("should throw an error if the device is not found", async () => {
        const data = "test-data";
        const invalidDeviceId = "invalid-device-id";
        await expect(signTransaction(invalidDeviceId, data, persistence)).rejects.toThrowError("Device not found");
    });
});
